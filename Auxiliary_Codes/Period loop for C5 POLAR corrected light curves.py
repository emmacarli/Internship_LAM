...
filepathlist=list(r"C:\Users\New\OneDrive - University of Glasgow\Work\Internship LAM\Figures\Polar\C5") #Path to the folder containing graphs for POLAR
...
while star: #Keep reading one line at a time till you get to the end of the list.
    ...
    print('For star',''.join(starlist)) #After this the stellar periods will be printed
    #Here we create the link to the POLAR txt file corresponding to this star
    linklist=list('https://archive.stsci.edu/hlsps/polar/c05/211800000/14413/hlsp_polar_k2_lightcurve_211814413-c05_kepler_v1_llc.fits') #Take a random link to a POLAR txt file and make it a list
    newlinklist = linklist[:42] + starlist[0:4] + linklist[46:52] + starlist[4:9] + linklist[57:83] + starlist +linklist[92:] #Replace the EPIC ID in the link using the stellar ID list
...
# =============================================================================    
    ...
    flux=arrays.data['DETFLUX'] #Retrieve the flux 
    time=arrays.data['DETTIME'] #Retrieve the timestamps
    ...
    ax.legend()
    ax.set_title('Comparing LTT and data (C5)')
    ...
    if n!=2: #If there is not just space for one period in the data
        ...
        if len(peaks)!=1: #If there is not just one peak in the ACF that is a multiple of the period
            ...
            ax1.legend()
            ax1.set_title('Eye-matching the periods (C5)')
    ...
    if n==2 or len(peaks)==1: #If there is space just for one period in the dataset, or if only one ACF period multiple could be found
        ...
        ax1.vlines(roughperiods, ymin, ymax, colors='b', linestyles='dashed', label='Rough periods') #Plot the rough period markers
        ax1.legend()
        ax1.set_title('Eye-matching the rough periods (C5)')
        ax1.set_xlabel('Time (BJD-2400000.0)')
        ax1.set_ylabel('Relative flux')
        ax1.set_xlim(time[0]-1,time[len(time)-1]) #Start x-axis a bit before so can see first vertical line.
        ax1.ticklabel_format(useOffset=False)
...
periods_table=Table(ID_and_periods,names=['ID','Rough Period C5','Rough Period Error C5','Period C5','Period Error C5']) #Make an astropy table with the data for easy handling
periods_table.write('periods_Polar_C5.html', overwrite=True) #Retrieve the astropy table as an html table

#TO MAYBE DO?
#Pass cadences (30 min) into data interpolating and interpolated ACF
#Sigma clipping, what sigma should I choose?
#Argrelmax in rough period
#Instead of using entriestime maybe use time-firstday



#NOTES
#A lag is just how much time has passed since the beginning (entriestime)
